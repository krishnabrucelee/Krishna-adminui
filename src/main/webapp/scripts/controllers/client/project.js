...
angular
        ...
        .controller('projectInfraLimitCtrl', projectInfraLimitCtrl)
        .controller('projectInfrastructureCtrl', projectInfrastructureCtrl)
        .controller('invoiceCtrl', invoiceCtrl)
        .controller('addInvoiceItemCtrl', addInvoiceItemCtrl)
        .controller('projectPaymentCtrl', projectPaymentCtrl)
        .controller('projectAddPaymentCtrl', projectAddPaymentCtrl)
        .controller('billingSummaryCtrl', billingSummaryCtrl)
        .controller('projectViewCtrl', projectViewCtrl)



function projectViewCtrl($scope, $state, $stateParams, $window, promiseAjax, modalService, notify) {
    ...
    $scope.confirmProjectCancellation = function () {
        modalService.trigger('views/client/project/confirm-cancellation.html', 'sm');
    };
    ...
    $scope.cancelProject = function () {
        ...
        setTimeout(function () {
            ...
            notify({message: 'Project cancelled successfully', classes: 'alert-success', templateUrl: $scope.homerTemplate});
    ...
    $scope.confirmProjectSuspension = function () {
        modalService.trigger('views/client/project/confirm-suspension.html', 'md');
    };
    ...
    $scope.suspendProject = function () {
        ...
        setTimeout(function () {
            $scope.cancel();
            $scope.homerTemplate = 'views/notification/notify.html';
            notify({message: 'Project suspended successfully', classes: 'alert-success', templateUrl: $scope.homerTemplate});
            $window.location.href = '#/client/project';
        }, 1000);
    };
...
function projectAddPaymentCtrl($scope, projectInfo, $modalInstance, notify) {
    ...
    $scope.addPayment = function (form) {
        ...
        if (form.$valid) {
            $scope.homerTemplate = 'views/notification/notify.html';
            notify({message: 'Payment added successfully', classes: 'alert-success', "timeOut": "1000", templateUrl: $scope.homerTemplate});
            $scope.cancel();
        }
    };
...
function addInvoiceItemCtrl($scope, $modalInstance, notify) {
    $scope.addPayment = function (form) {
        ...
        if (form.$valid) {
            $scope.homerTemplate = 'views/notification/notify.html';
            notify({message: 'Added successfully', classes: 'alert-success', "timeOut": "1000", templateUrl: $scope.homerTemplate});
            $scope.cancel();
        }
    };
...
function projectListCtrl($scope, promiseAjax) {
    $scope.projectList = {};
    var hasServer = promiseAjax.httpRequest("GET", "api/projects.json");
    hasServer.then(function (result) {  // this is only run after $http completes
